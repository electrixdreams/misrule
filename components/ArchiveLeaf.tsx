import type { AuditResultDto, FindingDto } from "@/lib/contracts";
...
export function ArchiveLeaf(props: Props) {
  const { pack, station, selectedSource, finding, result } = props;
  return (
    <article className="archive-leaf" id="archive-leaf" aria-label="Archive leaf">
      ...
      <div className="leaf-paper" tabIndex={-1} id="leaf-content">
        ...
        {finding ? (
          ...
          <>
            <LeafHeader eyebrow="Bundled world · synthetic demo pack" title={pack.title} folio="World leaf 01" />
            ...
            <div className="engraved-counts">
              ...
              <div><b>{pack.books.length}</b><span>mounted volume</span></div>
            </div>
          ...
          <>
            <LeafHeader eyebrow="World archive · declared constraints" title="World Rules" folio={`${pack.rules.length} complete axioms`} />
          ...
          <>
            <LeafHeader eyebrow="Narrative archive · surviving evidence" title="Narrative Record" folio={`${pack.spans.length} complete spans`} />
          ...
          <>
            <LeafHeader eyebrow="Validated alignments" title="Findings" folio={result ? `${result.findings.length} audit findings` : "Registry sealed"} />
          ...
          <>
            ...
            <div className="method-grid">
              ...
              <section><span>Live inference</span><p>The production route requests the provider and model selected in Settings through the server. A deterministic mock can exercise the same boundaries during QA, but it is not presented as live evidence.</p></section>
              ...
              <section><span>Product boundary</span><p>This is a synthetic World Pack demonstration, not an objective canon judge, manuscript uploader, final library/editor, graph database, or production-scale story system.</p></section>
